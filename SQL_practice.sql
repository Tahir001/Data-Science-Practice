...
SELECT COUNT(low) -- NOT NULL Values only. 
...
SELECT COUNT(date) AS num_dates, 
  COUNT(year) AS num_year, 
  COUNT(month) AS num_month, 
  COUNT(open) AS num_open, 
  COUNT(high) AS num_high, 
  COUNT(low) AS num_low, 
  COUNT(close) AS num_close, 
  COUNT(volume) AS num_volume, 
  COUNT(id) AS num_id
  FROM tutorial.aapl_historical_stock_price

-- We can see that num_high has the most null values (i.e least amount of rows). 

-- CASE WHEN PRACTICE
-- NORMAL STRUCTURE: CASE WHEN (right after select), colnam = condition, THEN condition END AS new_column
